...
export const cancelapplication = async (req, res) => {
    ...
    if (status === 'pending') {
        ...
        try {
            await db.promise().query(update_application_query, update_application_values)
            return res.status(200).json('Your request cancelled successfully')
        }
        catch (err) {
            console.log(err)
            return res.status(500).json('error occured!')
        ...
        const reporting_head_name_query = `select concat(first_name,' ', last_name) as name from usermanagement where email=? and status = 'active'`
        const reporting_head_name_value = [mail_approved_by]
        db.query(reporting_head_name_query, reporting_head_name_value, (error, result) => {
            if (error) return res.status(500).json('error occurred!')
            else {
                if (result.length === 0) {
